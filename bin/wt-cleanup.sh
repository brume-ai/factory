...
for dir in "$ROOT"/.worktrees/card-*; do
  ...
  name="$(basename "$dir")"
  card="${name#card-}"
  # LA DERNIÈRE LIGNE, PAS LA PREMIÈRE : une carte peut avoir eu plusieurs PR
  # (fermée puis rouverte proprement), et c'est la plus récente qui dit où en est
  # le travail. GitHub les rend dans l'ordre, donc `awk` garde la dernière vue.
