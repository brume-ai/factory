...
| `FACTORY_REVIEW_MAX` | `2` | `conf_get` | **N**, le nombre d'allers-retours du relecteur maintenabilité (un entier ≥ 1, sinon 3) : `bin/role.sh` refuse (code 5) de le lancer une fois de plus, `bin/turn-verify.sh` refuse le push d'un tour qui l'a dépassé, et refuse un `changements` au plafond sans `suite.md` (la carte de suite qui porte le reste exigé). Le plafond n'est pas un arbitrage humain : la dette part dans une carte, le code part |
