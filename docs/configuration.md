...
| `FACTORY_REVIEW_MAX` | `2` | `conf_get` | **N**, le nombre d'allers-retours du relecteur maintenabilité (un entier ≥ 1, sinon 3) : `bin/role.sh` refuse (code 5) de le lancer une fois de plus, `bin/turn-verify.sh` refuse le push d'un tour qui l'a dépassé, et refuse un `changements` au plafond sans `arbitrage.md` (le jugement de l'orchestrateur : `reprise` ouvre une passe N+1, la seule tolérée au-delà ; `suite` exige `suite.md`, la carte de suite ; `ok` écarte tout, motivé). Le plafond n'est pas un arbitrage humain : l'orchestrateur juge, le code part |
